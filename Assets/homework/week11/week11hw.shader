Shader "examples/week 11/homework"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : COLOR
            {
                ...
                float4 skyLerp = lerp(fogIntensity*fogColor, col, step(0.9999, depthValue));
                skyLerp += fogIntensity*fogColor;

                depthValue = easeOutSine(depthValue);
                float4 newLerp = lerp(col, skyLerp, depthValue);
            
                float4 output = newLerp;
                return output; 
            }
            ENDCG
        }
