Shader "examples/week 10/blinn-phong"
{
    Properties 
    {
        ...
        [NoScaleOffset] _IBL ("IBL cube map", Cube) = "black" {}
         _mipLevel ("mip level",Range(0,12)) = 5
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (Interpolators i) : SV_Target
            {
                ...
                float3 viewReflection = reflect(-viewDirection, normal);
                 ...
                 float3 indirectDiffuse = texCUBElod(_IBL,float4(normal,_mipLevel));

                 float3 diffuse = _surfaceColor * (directDiffuse * lightColor + indirectDiffuse);
                float3 specular = pow(specularFalloff, _gloss * MAX_SPECULAR_POWER + 0.0001) * lightColor * _gloss;
                float3 blinnphong = diffuse + specular;
